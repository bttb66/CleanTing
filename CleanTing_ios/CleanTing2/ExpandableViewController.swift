...
class ExpandableViewController: UIViewController{
    ...
    @IBAction func Create_Ting_Action(_ sender: Any) {
        ...
        model.generate_Ting(userId: gsno(userId), date: gsno(r_calendar_data), startTime: gsno(r_params_string), endTime: gsno(r_params_string_end), price: "13000", cleanerId: "bumjin", request: gino(r_params), warning: gsno(r_warning), lat: gdno(Double(lat!)), lng: gdno(Double(lng!)))
    ...
    //날짜
    var r_calendar_data : String = "날짜선택"
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        //청소날짜
        self.tableView.register(UINib(nibName: "CalendarTableViewCell", bundle: nil), forCellReuseIdentifier: "cal_cell")
    ...
    func keyboardShow(){
        ...
        UIView.animate(withDuration: 0.5, delay: 0, usingSpringWithDamping: 1, initialSpringVelocity: 1 ,options: .curveEaseOut, animations: {
            self.view.frame = CGRect(x:0, y:-180, width:self.view.frame.width, height:self.view.frame.height)
        }, completion: nil)
...
//            cell.btn830.addTarget(self, action: #selector(select_time(_:)), for: .touchUpInside)
        ...
        if indexPath.section == 2 && indexPath.item == 0 {
            ...
            let cell = tableView.dequeueReusableCell(withIdentifier: "cal_cell", for: indexPath) as! CalendarTableViewCell
            
            cell.selectionStyle = .none
            cell.accessoryType = .disclosureIndicator
           
            cell.calendar_label.text = self.r_calendar_data
            cell.action.addTarget(self, action: #selector(move_choose_view(_:)), for: .touchUpInside)
